...
export const realEstateSeedData: OnboardingFieldSchema[] = [
  // ============================================================
  // CATEGORY: general  (Core Identity)
  // ============================================================
  {
    field_key: 'company_name',
    field_name: 'Company Name',
    field_description: 'Official builder or real estate developer name',
    field_type: 'string',
    required: true,
    extraction_hint: 'Extract the exact real estate developer name from letterheads, headers, or "about us" sections.',
    example_value: 'Sahu Builders',
    category: 'general',
    display_order: 1,
  },
  {
    field_key: 'business_description',
    field_name: 'Business Overview',
    field_description: 'Concise summary of the builder’s legacy, focus, and reputation',
    field_type: 'string',
    required: true,
    extraction_hint: 'Extract the company overview, years in business, and core philosophy (e.g., luxury, affordable, eco-friendly). Summarize in 2-3 sentences.',
    example_value: 'Premier real estate development company specializing in luxury and affordable residential projects with a 20-year legacy of timely delivery.',
    category: 'general',
    display_order: 2,
  },

  // ============================================================
  // CATEGORY: services  (Property Portfolio & Purchasing)
  // ============================================================
  {
    field_key: 'property_portfolio',
    field_name: 'Active Projects / Property Portfolio',
    field_description: 'Detailed list of all active real estate projects available for sale',
    field_type: 'json',
    required: true,
    extraction_hint: 'Extract every real estate project mentioned. Return an array of objects containing: project_name, location, configurations (e.g., 2 BHK, Villas), base_price_range, amenities, and possession_date.',
    example_value: JSON.stringify([
      {
        project_name: 'The Sapphire Heights',
        location: 'Downtown Metro Sector 45',
        configurations: '3 BHK & 4 BHK Luxury Apartments',
        base_price_range: '$450,000 to $800,000',
  ...
  {
    field_key: 'payment_and_financing',
    field_name: 'Payment Plans & Financing Options',
    field_description: 'Available payment schemes, subvention plans, and loan assistance',
    field_type: 'array',
    required: true,
    extraction_hint: 'Extract all payment plans (e.g., Construction Linked Plan, 20:80 scheme) and information about home loan assistance or pre-approved banks. Return as an array of strings.',
    example_value: JSON.stringify(['Construction Linked Plan (CLP): 10% on booking', 'Zero-cost in-house home loan assistance']),
    category: 'services',
    display_order: 11,
  },
  {
    field_key: 'booking_and_cancellation',
    field_name: 'Booking & Cancellation Policy',
    field_description: 'Rules regarding token amounts, refunds, and cancellations',
    field_type: 'string',
    required: true,
    extraction_hint: 'Summarize the exact token amount required to book a unit, the timeline for signing agreements, and the penalty/refund policy for cancellations.',
    example_value: 'Token amount is $5,000. 100% refund if cancelled within 30 days of token payment; otherwise, 5% of property cost is deducted.',
    category: 'services',
    display_order: 12,
  },
  {
    field_key: 'faqs',
    field_name: 'Frequently Asked Questions',
    field_description: 'Common customer questions and official answers',
    field_type: 'json',
    required: false,
    extraction_hint: 'Extract any explicit Q&A blocks regarding customization, site visits, or general property inquiries. Return as an array of { question: string, answer: string }.',
    example_value: JSON.stringify([
      { question: 'Can I customize the interior?', answer: 'Customizations are strictly prohibited for high-rise projects.' }
    ]),
    category: 'services',
    display_order: 13,
  },
  ...
  // ============================================================
  // CATEGORY: ai_config  (Agent Behavior & Conversion Goals)
  // ============================================================
  {
    field_key: 'primary_conversion_goal',
    field_name: 'Primary AI Agent Goal',
    field_description: 'The main objective the AI agent must drive the customer toward',
    field_type: 'string',
    required: true,
    extraction_hint: 'Look for the primary CTA or goal for the agent, such as "schedule a site visit", "book an appointment", or "collect lead info".',
    example_value: 'The primary goal is to schedule an in-person site visit between 10:00 AM and 6:00 PM.',
    category: 'ai_config',
    display_order: 20,
  },
  {
    field_key: 'strict_agent_protocols',
    field_name: 'Strict Behavioral Protocols & Triage',
    field_description: 'Rules the agent must never break (e.g., pricing negotiation rules)',
    field_type: 'array',
    required: true,
    extraction_hint: 'Extract strict rules, safety triages, or boundaries. Look for "CRITICAL TRIAGE", "agent must STOP", "never badmouth", or rules about handling discount requests.',
    example_value: JSON.stringify([
      'Never negotiate prices or offer discounts over chat.',
      'If asked for a discount, reply exactly: "Our best pricing is available at the sales gallery. Would you like to schedule a site visit?"',
      'Never badmouth competitor builders.'
    ]),
    category: 'ai_config',
    display_order: 21,
  }
...
export async function seedOnboardingFieldSchemas() {
  for (const item of realEstateSeedData) {
    await pool.query(
      `
      INSERT INTO onboarding_field_schemas (
        field_key, field_name, field_description, field_type, 
        required, extraction_hint, example_value, category, display_order
      )
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      ON CONFLICT (field_key) DO NOTHING;
      `,
      [
        item.field_key, item.field_name, item.field_description, item.field_type,
        item.required, item.extraction_hint, item.example_value, item.category, item.display_order
      ]
    );
  }
  console.log('Real Estate Onboarding field schemas seeded successfully');
}